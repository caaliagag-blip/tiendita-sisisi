...
document.addEventListener('DOMContentLoaded', () => {
  if (document.getElementById('favorite-count')) updateFavoriteCount();
  if (document.getElementById('product-grid')) renderProducts(products);

  // Event Listeners para Filtros (registrar solo si existen los elementos)
  ...
  // Actualizar UI del carrito para reflejar descuento cargado desde localStorage
  updateCartUI();
});
...
function handleRegister(e) {
  ...
  if (email.endsWith('@duocuc.cl') || email.endsWith('@profesor.duoc.cl')) {
    ...
    localStorage.setItem('level-up-duoc', '1');
    ...
    localStorage.setItem('level-up-duoc', '0');
...
function updateCartUI() {
  ...
  // Cálculos de Totales (actualizar solo si existen los elementos)
  ...
  if (elDiscount) elDiscount.textContent = `-$${discount.toLocaleString('es-CL')} CLP`;
